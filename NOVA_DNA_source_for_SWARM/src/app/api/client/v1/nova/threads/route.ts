...
import { listNovaMobileThreads } from "@/lib/nova/mobile/threads-service";
...
/**
 * GET /api/client/v1/nova/threads — inbox rows (NOVA + Updates).
 */
export async function GET(req: Request) {
  const auth = await requireClientApiUser("ai.assistant.read", req, METHODS);
  if (auth.error) return auth.error;

  const result = await listNovaMobileThreads(auth.user);
  if (!result.ok) {
